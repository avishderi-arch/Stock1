...
MIDCAP_GROWTH = [
    "SNOW","PLTR","NET","BILL","GTLB","MDB","CFLT","SMAR","DUOL","ASAN",
    ...
    try:
        ...
        for sector,(etf,stocks) in SECTOR_ETFS.items():
            try:
                ...
                r1m=safe_float(df["Close"].pct_change(21).iloc[-1])
                r5d=safe_float(df["Close"].pct_change(5).iloc[-1])
...
def analyse_stock(ticker, spy_close, strict=True, sector_score=0):
    try:
        ...
        comp=clamp(base+vwap_bonus+sec_bonus+h52_bonus/10+trend_bonus/10)
...
def ai_deep_analysis(data, api_key, regime=None, sectors=None):
    ...
    if sectors:
        ...
        sector_ctx = ""
    prompt=f"""אתה אנליסט מוסדי מקצועי. נתח מניה לסווינג 5-10 ימים. ענה בעברית בלבד. היה ספציפי ומעשי.
...
RSI: {data['rsi']} | MFI: {data['mfi']} | ADX: {data['adx']} | MACD: {data['macd_hist']}
Vol: {data['vs']}x | OBV: {"עולה" if data['obv_up'] else "יורד"} | vs SPY: {data['rs']}%
מומנטום 1M: {data['m1']}% | 3M: {data['m3']}% | ATR: {data['atr_pct']}%
שיא 52W: ${data['high52']} | מרחק: {data['dist_52h']}% | טרנד: {data['trend_age']} ימים
...
def show_card(i, row, style="normal", show_earnings=False):
    ...
    st.markdown(f"""<div class="{cls}">
      ...
      {'<div style="font-size:0.75rem;margin:4px 0;color:#f0b429">'+flags+'</div>' if flags else ''}
      <div class="row-item">💰 <span class="row-val">${row['price']}</span> | VWAP: <span class="{vc}">{vi} ${row.get('vwap','-')} ({row.get('vwap_dist',0):+.1f}%)</span></div>
      <div class="row-item">📈 3M: <span class="{m3c}">{arrow}{abs(row['m3'])}%</span> | Vol: <span class="row-val">{row['vs']}x</span> | ADX: <span class="row-val">{row['adx']}</span> | טרנד: <span class="row-val">{row.get('trend_age',0)}d</span></div>
      ...
      <div class="row-item">📏 שיא 52W: <span class="row-val">{h52_str}</span> (מרחק: {row.get('dist_52h','N/A')}%)</div>
      ...
      <div class="row-item">📝 <span style="color:#c9d6df">{row['setup']}</span></div>
...
def main():
    ...
    if mode=="🔍 סריקה Top-Down (מומלץ)":
        ...
        if st.button("🔍 סרוק Top-Down"):
            ...
            # For full SP500, top_secs may not be defined
            if "top_secs" not in dir():
                top_secs = sectors[:n_sectors]
            prog=st.progress(0); rs=[]; rr=[]
            for i,t in enumerate(all_t):
                t_sec=next((s["sector"] for s in SECTOR_ETFS.items() if t in s[1][1]),"") if hasattr(SECTOR_ETFS,"items") else ""
                # simpler sector lookup
                t_sec2=next((sec for sec,(etf,stks) in SECTOR_ETFS.items() if t in stks),"")
                sc=sec_map.get(t_sec2,0)
            ...
            for i,row in enumerate(results[:top_n]): show_card(i,row,show_earnings=show_earn)
    ...
    # ── SINGLE + AI ───────────────────────────────────────────
        ...
        if st.button("🎯 נתח"):
            ...
            with st.spinner("טוען נתוני שוק..."): regime=get_market_regime(); sectors=get_sector_ranking()
            show_regime(regime)
            spy_c=get_spy_close()
            ...
            if api_key:
                st.markdown("---"); st.markdown("### 🤖 ניתוח AI מעמיק")
            ...
            else: st.info("💡 הוסף מפתח API לניתוח AI")
