...
st.markdown("""
<style>
@import url('https://fonts.googleapis.com/css2?family=IBM+Plex+Mono:wght@400;500;700&family=IBM+Plex+Sans:wght@400;600&display=swap');
html, body, [class*="css"] { background:#080b12 !important; color:#e6edf3 !important; font-family:'IBM Plex Mono',monospace; }
.main-title { color:#21ff87; font-size:1.9rem; font-weight:700; text-align:center; margin-bottom:2px; letter-spacing:-0.5px;
  text-shadow: 0 0 20px rgba(33,255,135,0.25); }
.sub-title  { color:#7d8590; font-size:0.8rem; text-align:center; margin-bottom:14px; font-family:'IBM Plex Sans',sans-serif; }
.stButton > button {
    background:linear-gradient(135deg,#21ff87 0%,#00d4ff 100%) !important;
    color:#06090f !important; font-weight:700 !important; font-size:1.02rem !important;
    border:none !important; border-radius:12px !important; padding:14px !important;
    width:100% !important; margin:8px 0 !important;
    box-shadow: 0 4px 14px rgba(33,255,135,0.18) !important;
    transition: transform 0.15s ease, box-shadow 0.15s ease !important;
}
.stButton > button:active { transform: scale(0.98) !important; }
.regime-bull { background:linear-gradient(145deg,#0d1a10,#0a1410); border:1px solid #21ff8755; border-left:4px solid #21ff87; border-radius:12px; padding:14px 16px; margin:8px 0; color:#e6edf3; }
.regime-bear { background:linear-gradient(145deg,#1a0d0d,#140a0a); border:1px solid #ff7b7255; border-left:4px solid #ff7b72; border-radius:12px; padding:14px 16px; margin:8px 0; color:#e6edf3; }
.regime-neut { background:linear-gradient(145deg,#1a170d,#14120a); border:1px solid #f0b42955; border-left:4px solid #f0b429; border-radius:12px; padding:14px 16px; margin:8px 0; color:#e6edf3; }
.sector-card { background:#11151d; border:1px solid #22283355; border-radius:10px; padding:10px 14px; margin:4px 0; color:#e6edf3; font-size:0.85rem; }
.result-card { background:#11151d; border:1px solid #22283399; border-radius:14px; padding:16px; margin:9px 0; color:#e6edf3;
  box-shadow: 0 2px 8px rgba(0,0,0,0.25); }
.hc-card     { background:linear-gradient(145deg,#0d1512,#0a1210); border:1.5px solid #21ff87aa; border-radius:16px; padding:16px; margin:10px 0; color:#e6edf3;
  box-shadow: 0 4px 16px rgba(33,255,135,0.1); }
.ss-card     { background:linear-gradient(145deg,#150d15,#120a12); border:1.5px solid #ff7b72aa; border-radius:16px; padding:16px; margin:10px 0; color:#e6edf3; }
.gap-card    { background:linear-gradient(145deg,#0d1515,#0a1212); border:1.5px solid #00d4ffaa; border-radius:16px; padding:16px; margin:10px 0; color:#e6edf3; }
.value-card  { background:linear-gradient(145deg,#15130d,#12100a); border:1.5px solid #f0b429aa; border-radius:16px; padding:16px; margin:10px 0; color:#e6edf3; }
.ai-card {
    background:#0d1117; border:1px solid #21ff8740;
    border-right:4px solid #21ff87; border-radius:14px;
    padding:20px; margin:8px 0; font-size:0.9rem;
    line-height:1.85; color:#e6edf3 !important;
...
.earnings-warn { background:#1a1500; border:1px solid #f0b429; border-radius:10px; padding:8px 12px; margin:4px 0; font-size:0.78rem; color:#f0b429; }
.ticker   { color:#58a6ff; font-size:1.18rem; font-weight:700; letter-spacing:-0.3px; }
...
.disclaimer { background:#11151d; border-left:3px solid #ff7b72; border-radius:10px; padding:12px; margin-top:16px; font-size:0.72rem; color:#8b949e; }
.tracker-row { background:#11151d; border-radius:10px; padding:10px; margin:4px 0; font-size:0.8rem; color:#e6edf3; }
div[data-baseweb="radio"] label { background:#11151d; border-radius:10px; padding:6px 10px; margin:2px 0; }
...
# ── Near-Miss Detector (stocks that almost passed strict filters) ──
def analyse_near_miss(ticker, spy_close, sector_score=0):
    """
    Run the same computation as strict analyse_stock, but instead of hard-failing,
    count how many of the 11 gates passed and return a 'gate_score' (0-11).
    Used to surface stocks that are 1-2 gates away from a full pass.
    """
    try:
        import time
        time.sleep(0.1)
        ...
        mh=macd_hist(c); mh_v=safe_float(mh.iloc[-1])
        ...
        failed_gates = [k for k,v_ in gates.items() if not v_]

        # Only interesting if 8+ of 11 gates pass (near miss) but not all 11
        if passed < 8 or passed == 11:
            return None

        r,_ = analyse_stock(ticker, spy_close, strict=False, sector_score=sector_score)
        if not r: return None
        r["gates_passed"] = passed
        r["gates_total"] = 11
        r["failed_gates"] = failed_gates
        return r
    except: return None

GATE_LABELS_HE = {
    "price":"מחיר מינימלי","vol":"נפח מינימלי","above_sma50":"מעל SMA50",
    "golden_cross":"Golden Cross","rsi_range":"טווח RSI","macd_pos":"MACD חיובי",
    "adx":"ADX מספיק","vol_surge":"נפח חריג","obv_up":"OBV עולה",
    "mfi":"MFI מספיק","rs_spy":"עדיף מ-SPY",
}

def show_near_miss_card(i, row):
    passed=row.get("gates_passed",0); total=row.get("gates_total",11)
    failed=row.get("failed_gates",[])
    failed_he=", ".join([GATE_LABELS_HE.get(g,g) for g in failed])
    pct=round(passed/total*100)
    color = "#f0b429" if passed>=9 else "#8b949e"
    show_card(i,row,style="normal")
    st.markdown(
        f'<div style="background:#161b22;border:1px dashed {color};border-radius:8px;'
        f'padding:8px 12px;margin:2px 0 10px;font-size:0.78rem;color:{color}">'
        f'🔶 קרוב לפריצה: עבר {passed}/{total} שערים ({pct}%) — נכשל ב: {failed_he}'
        f'</div>', unsafe_allow_html=True
    )



# ── Oversold Quality Bounce Scanner ───────────────────────────
def analyse_bounce(ticker, spy_close):
    """
    מחפש מניות איכותיות (מעל SMA200, טרנד ארוך טווח בריא) שספגו ירידה חדה
    בטווח הקצר אך המבנה הארוך עדיין שלם — מועמדות להתאוששות (mean reversion).
    """
    try:
        import time
        time.sleep(0.1)
        ...
        if price<=0: return None
        avg_vol=safe_float(v.rolling(20).mean().iloc[-1])
        if avg_vol<300000: return None

        sma200=safe_float(c.rolling(200).mean().iloc[-1])
        sma50=safe_float(c.rolling(50).mean().iloc[-1])
        rsi_v=safe_float(rsi(c).iloc[-1])
        atr_v=safe_float(calc_atr(h,l,c).iloc[-1])
        drop_5d=safe_float(c.pct_change(5).iloc[-1])
        drop_10d=safe_float(c.pct_change(10).iloc[-1])
        obv=(np.sign(c.diff()).fillna(0)*v).cumsum()
        obv_s_5=slope(obv.tail(5))     # short-term OBV — is selling slowing?
        obv_s_20=slope(obv.tail(20))   # medium-term OBV trend
        vs=safe_float(v.iloc[-1]/avg_vol) if avg_vol>0 else 0

        # Quality filter: long-term structure must still be intact
        quality_ok = price > sma200 * 0.95  # allow small undercut, not a full breakdown
        long_term_healthy = sma200 > 0 and safe_float(c.pct_change(126).iloc[-1]) > -0.05  # not in a 6mo downtrend

        # Sharp short-term drop
        sharp_drop = (rsi_v < 38) and (drop_5d < -0.06 or drop_10d < -0.10)

        if not (quality_ok and long_term_healthy and sharp_drop):
            return None

        # Stabilization signals (early reversal clues)
        stabilizing = obv_s_5 >= 0 or vs > 1.3  # selling pressure easing OR volume spike (capitulation/reversal)

        al=spy_close.reindex(c.index,method="ffill")
        rs_spy_20=safe_float(c.pct_change(20).iloc[-1])-safe_float(al.pct_change(20).iloc[-1]) if len(al)>=21 else 0

        # Bounce score
        score=0; reasons=[]
        if rsi_v<30: score+=25; reasons.append(f"RSI קיצוני ({rsi_v:.0f})")
        elif rsi_v<38: score+=15; reasons.append(f"RSI נמוך ({rsi_v:.0f})")
        if price>sma200: score+=20; reasons.append("מעל SMA200 — מבנה שלם")
        else: score+=8; reasons.append("קרוב ל-SMA200")
        if drop_5d<-0.10: score+=20; reasons.append(f"נפילה חדה 5D {drop_5d*100:.1f}%")
        elif drop_5d<-0.06: score+=12; reasons.append(f"ירידה 5D {drop_5d*100:.1f}%")
        if obv_s_5>=0: score+=15; reasons.append("OBV מתייצב")
        if vs>1.5: score+=10; reasons.append(f"נפח קפיצה {vs:.1f}x (יתכן capitulation)")
        if long_term_healthy: score+=10; reasons.append("טרנד 6M שלם")

        if score<40: return None

        stop = min(safe_float(l.tail(5).min()), price*0.94)
        risk = max(price-stop, 0.01)
        target1 = price + 1.5*risk
        target2 = sma50 if sma50>price else price+2.5*risk

        return {
            "t": ticker, "price": round(price,2), "bounce_score": min(round(score),100),
            "rsi": round(rsi_v,1), "drop_5d": round(drop_5d*100,1), "drop_10d": round(drop_10d*100,1),
            "vs": round(vs,2), "above_sma200": price>sma200,
            "sma200": round(sma200,2), "sma50": round(sma50,2),
            "obv_stabilizing": obv_s_5>=0, "rs_spy": round(rs_spy_20*100,1),
            "reasons": "; ".join(reasons[:4]),
            "stop": round(stop,2), "target1": round(target1,2), "target2": round(target2,2),
        }
    except: return None

def show_bounce_card(i, row):
    sc=row["bounce_score"]
    c = "#21ff87" if sc>=65 else "#f0b429" if sc>=45 else "#8b949e"
    structure = "✅ מבנה שלם (מעל SMA200)" if row["above_sma200"] else "⚠️ קרוב ל-SMA200"
    stab = "✅ OBV מתייצב" if row["obv_stabilizing"] else "⚪ עדיין במכירה"
    html = (
        f'<div style="background:#0d1117;border:2px solid {c};border-radius:12px;padding:14px;margin:8px 0;color:#e6edf3">'
        f'<div style="display:flex;justify-content:space-between;align-items:center">'
        f'<span class="ticker">#{i+1} {row["t"]} 🩹</span>'
        f'<span style="color:{c};font-weight:700">Bounce: {sc}</span></div>'
        f'<div class="row-item">💰 <span class="row-val">${row["price"]}</span> | RSI: <span class="row-val">{row["rsi"]}</span> | {structure}</div>'
        f'<div class="row-item">📉 5D: <span class="negative">{row["drop_5d"]:+.1f}%</span> | 10D: <span class="negative">{row["drop_10d"]:+.1f}%</span> | Vol: <span class="row-val">{row["vs"]}x</span></div>'
        f'<div class="row-item">{stab} | SMA50: <span class="row-val">${row["sma50"]}</span> | SMA200: <span class="row-val">${row["sma200"]}</span></div>'
        f'<div class="row-item">🛡️ סטופ: <span class="negative">${row["stop"]}</span> | יעד 1 (מהיר): <span class="positive">${row["target1"]}</span> | יעד 2 (SMA50): <span class="positive">${row["target2"]}</span></div>'
        f'<div class="row-item">📝 {row["reasons"]}</div>'
        f'</div>'
    )
...
def get_conviction_data(ticker):
    ...
    try:
        ...
        inst_pct = info.get("heldPercentInstitutions", 0) or info.get("institutionOwnershipPercent", 0) or info.get("institutionsPercentHeld", 0)
        if inst_pct:
...
# ── AI Analysis ───────────────────────────────────────────────
def ai_deep_analysis(data, api_key, regime=None, sectors=None, image_b64=None, media_type=None):
...
{"בנוסף לנתונים המספריים, מצורף צילום מסך עדכני של גרף המניה עם אינדיקטורים טכניים — התבסס גם עליו בניתוח (מגמה חזותית, תבניות מחיר, מיקום ביחס לממוצעים נעים) ושלב אותו עם הנתונים המספריים למטה." if image_b64 else ""}
...
## 📋 סיכום בשורה אחת"""
    ...
    try:
        resp=requests.post("https://api.anthropic.com/v1/messages",
            ...
            json={"model":"claude-sonnet-4-5","max_tokens":3000,"messages":[{"role":"user","content":content}]},
            timeout=120)
...
def run_ai_panel(r, api_key, regime=None, sectors=None):
    ...
    if api_key:
        ...
        # שומר את ההקשר כדי שאפשר יהיה להעלות גרף ולנתח מחדש
        # בסקשן נפרד (render_chart_reanalysis) - גם אחרי שכפתור
        # "נתח" כבר לא לחוץ, כי סטרימליט מאפס כפתורים בכל rerun.
        st.session_state["last_analysis"]={
            "ticker":r["ticker"],"r":r,"regime":regime,"sectors":sectors,"api_key":api_key,
        }
    else:
        st.info("💡 הוסף מפתח API לניתוח AI")

def render_chart_reanalysis():
    """
    אזור קבוע (מחוץ ללוגיקת הכפתורים) שמאפשר להעלות גרף ולנתח
    מחדש את הניתוח האחרון. חייב לחיות מחוץ ל-'if st.button(...)'
    כי העלאת קובץ גורמת ל-rerun של כל הסקריפט, וב-rerun הזה
    הכפתור המקורי כבר לא "לחוץ" - לכן נשען על session_state.
    """
    ctx=st.session_state.get("last_analysis")
    if not ctx or not ctx.get("api_key"):
        return
    st.markdown("---")
    st.markdown(f"### 📊 חידוד הניתוח עם גרף עדכני — {ctx['ticker']}")
    chart_file=st.file_uploader(
        "צילום מסך של הגרף (TradingView / כל אתר אחר)",
        type=["png","jpg","jpeg"], key=f"chart_upload_{ctx['ticker']}")
    if chart_file is not None:
        st.image(chart_file,caption=f"גרף {ctx['ticker']} שהועלה",use_container_width=True)
        if st.button("🔍 נתח מחדש עם הגרף",key=f"reanalyze_{ctx['ticker']}"):
            image_b64=base64.b64encode(chart_file.getvalue()).decode("utf-8")
            media_type=chart_file.type or "image/png"
            with st.spinner("Claude מנתח מחדש עם הגרף... (עד 2 דקות)"):
                analysis=ai_deep_analysis(
                    ctx["r"],ctx["api_key"],regime=ctx["regime"],sectors=ctx["sectors"],
                    image_b64=image_b64,media_type=media_type)
            html=re.sub(r'^## (.+)$',r'<h2>\1</h2>',analysis,flags=re.MULTILINE)
            html=re.sub(r'^### (.+)$',r'<h3>\1</h3>',html,flags=re.MULTILINE)
            html=html.replace("\n","<br>")
            st.markdown(f'<div class="ai-card">{html}</div>',unsafe_allow_html=True)
            st.markdown("**📋 העתק ניתוח מעודכן (כולל הגרף):**")
            st.code(analysis,language=None)
...
def main():
    ...
    if mode=="🔍 סריקה Top-Down (מומלץ)":
        ...
        if st.button("🔍 סרוק Top-Down"):
            ...
            st.session_state["td_results"]=results[:top_n]
            save_tracker(results[:top_n])

            # Near-miss pass: from the "relaxed" pool (rr), find ones close to passing all gates
            with st.spinner("מחפש מועמדות שקרובות לפריצה (Near Miss)..."):
                nm_pool = rr[:30] if rr else results[top_n:top_n+30]
                near_misses=[]
                for row in nm_pool:
                    t_sec = row.get("sector","")
                    sc = sec_map.get(t_sec,0)
                    nm = analyse_near_miss(row["ticker"], spy_c, sector_score=sc)
                    if nm: near_misses.append(nm)
                near_misses.sort(key=lambda x:(x.get("gates_passed",0),x["score"]),reverse=True)
                st.session_state["near_miss_results"]=near_misses[:3]
        ...
        # ── הצגת תוצאות (מחוץ לבלוק הסריקה — נשמר ב-session_state) ──
        if st.session_state.get("td_results"):
            for i,row in enumerate(st.session_state["td_results"]):
                show_card(i,row,show_earnings=row.get("earnings_days") is not None)
                if row.get("upgrade_bonus") or row.get("upgrades"):
            ...
            if st.session_state.get("near_miss_results"):
                st.markdown("---")
                st.markdown("### 🔶 קרובות לפריצה (Near Miss)")
                st.caption("מניות שעברו 8-10 מתוך 11 שערי סינון — כדאי לעקוב, לא בהכרח לקנות עכשיו")
                for i,row in enumerate(st.session_state["near_miss_results"]):
                    show_near_miss_card(i,row)
    ...
    # ── OVERSOLD BOUNCE ───────────────────────────────────────
    elif mode=="🩹 Oversold Bounce":
        opt=st.radio("Universe:",["🚀 סקטורים","➕ כולל צמיחה","💎 S&P 500 מלא"])
        st.info("🎯 מחפש מניות איכותיות (מבנה ארוך-טווח שלם) שספגו ירידה חדה בטווח הקצר — הזדמנות התאוששות (mean reversion). מתאים ל-3-7 ימי החזקה.")
        st.caption("קריטריונים: RSI<38 + ירידה חדה ב-5-10 ימים + עדיין מעל/קרוב ל-SMA200 + סימני התייצבות ב-OBV או נפח")

        if st.button("🩹 סרוק Oversold Bounce"):
            if "מלא" in opt:
                with st.spinner("טוען S&P 500 מ-Wikipedia..."):
                    tickers = load_sp500_full()
                if not tickers:
                    tickers = list(dict.fromkeys([t for s in SECTOR_ETFS.values() for t in s[1]]))
            elif "צמיחה" in opt:
                tickers=list(dict.fromkeys([t for s in SECTOR_ETFS.values() for t in s[1]]+MIDCAP_GROWTH))
            ...
            spy_c=get_spy_close()
            prog=st.progress(0); bounces=[]
            for i,t in enumerate(tickers):
                r=analyse_bounce(t,spy_c)
                if r: bounces.append(r)
                prog.progress((i+1)/len(tickers),text=f"סורק {t}...")
            prog.empty()
            bounces.sort(key=lambda x:x["bounce_score"],reverse=True)
            st.session_state["bounce_results"]=bounces[:8]

        if st.session_state.get("bounce_results"):
            br=st.session_state["bounce_results"]
            if br:
                st.success(f"🩹 {len(br)} מועמדות להתאוששות!")
                st.warning("⚠️ אלו מניות בירידה חדה — ודא שאין קטליזטור שלילי מהותי (דוחות, חדשות רעות) לפני כניסה.")
                for i,row in enumerate(br): show_bounce_card(i,row)
            else:
                st.warning("לא נמצאו מועמדות כרגע.")
