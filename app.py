...
def main():
    ...
    if mode == "🔍 סריקה כללית — S&P 500":
        ...
        if "QQQ" in universe_size:
            tickers = QQQ100
        elif "500 מניות" in universe_size:
            tickers = SP500_FULL
        else:
            tickers = SP500_FULL[:150]
        ...
        if st.button("🔍  RUN SCAN — סרוק עכשיו"):
            ...
            for i,t in enumerate(tickers):
                ...
                if r:
                    ...
                    r2,_ = analyse(t, spy_c, strict=False)
                    if r2 and r2["score"] >= 55:
                        r2["relaxed"] = True
                        results_relaxed.append(r2)
                prog.progress((i+1)/len(tickers), text=f"סורק {t}... ({i+1}/{len(tickers)})")
            prog.empty()

            if results_strict:
                results = results_strict
                st.success(f"✅ נמצאו {len(results_strict)} מניות שעוברות את כל הפילטרים!")
            elif results_relaxed:
                results = results_relaxed
                st.warning(f"⚠️ לא נמצאו מניות עם כל הפילטרים. מציג {len(results_relaxed)} מניות עם ניקוד גבוה (פילטרים מקולים).")
            else:
                st.warning("לא נמצאו מניות מתאימות היום. השוק ייתכן שסגור או שהתנאים חלשים.")
                return
    ...
    # ── Mode 2: Single stock + AI ─────────────────────────────
        ...
        if st.button("🔍  נתח עכשיו"):
            ...
            if api_key:
                ...
                with st.spinner("Claude מנתח את המניה... (עד 2 דקות)"):
