...
def run_ai_panel(r, api_key, regime=None, sectors=None):
    ...
    with st.expander(f"📊 העלה גרף עדכני של {r['ticker']} (אופציונלי)"):
        chart_file=st.file_uploader("צילום מסך של הגרף — TradingView / כל אתר אחר",
            type=["png","jpg","jpeg"],key=f"chart_upload_{r['ticker']}")
        if chart_file is not None:
            st.image(chart_file,caption=f"גרף {r['ticker']} שהועלה",use_container_width=True)
    ...
    if api_key:
        ...
        spinner_msg="Claude מנתח (כולל הגרף שהועלה)... (עד 2 דקות)" if image_b64 else "Claude מנתח... (עד 2 דקות)"
        with st.spinner(spinner_msg):
            analysis=ai_deep_analysis(r,api_key,regime=regime,sectors=sectors,image_b64=image_b64,media_type=media_type)
            if "שגיאת חיבור" in analysis or "timed out" in analysis.lower():
                st.info("מנסה שוב..."); analysis=ai_deep_analysis(r,api_key,regime=regime,sectors=sectors,image_b64=image_b64,media_type=media_type)
