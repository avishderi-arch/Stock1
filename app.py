...
def main():
    ...
    if st.session_state.get("ai_prefill") and st.session_state.get("force_ai_mode"):
        st.session_state["selected_mode"]="🎯 מניה בודדת + AI"
        st.session_state["force_ai_mode"]=False
    if "selected_mode" not in st.session_state:
        st.session_state["selected_mode"]=MODES[0]
        ...
        if st.button("🔍 סרוק Top-Down"):
            ...
            for i,row in enumerate(results[:top_n]):
                ...
                if st.button(f"🤖 נתח {row['ticker']} עם AI", key=f"ai_td_{row['ticker']}_{i}"):
                    st.session_state["ai_prefill"]=row
                    st.session_state["ai_regime"]=st.session_state.get("last_regime")
                    st.session_state["ai_sectors"]=st.session_state.get("last_sectors")
                    st.session_state["force_ai_mode"]=True
                    st.rerun()
            ...
            if hc:
                ...
                for i,row in enumerate(hc):
                    ...
                    if st.button(f"🤖 נתח {row['ticker']} עם AI", key=f"ai_hc_{row['ticker']}_{i}"):
                        ...
                        st.session_state["ai_sectors"]=None
                        st.session_state["force_ai_mode"]=True
                        st.rerun()
            elif all_r:
    ...
    # ── SINGLE + AI ───────────────────────────────────────────
        ...
        if prefill:
            st.success(f"✅ נטען מהסורק: **{prefill['ticker']}** (ניקוד: {prefill['score']}) — ניתן לשנות או לנתח ישירות")

        default_ticker=prefill["ticker"] if prefill else ""
        ticker_input=st.text_input("הכנס טיקר:",default_ticker).strip().upper()
        api_key=st.text_input("מפתח API:",type="password")
        show_earn=st.checkbox("בדוק דוח רווחים",value=True)

        col1,col2=st.columns([1,1])
        with col1: run_analysis=st.button("🎯 נתח")
        with col2:
            if prefill and st.button("🗑️ נקה בחירה מסורק"):
                del st.session_state["ai_prefill"]
                st.rerun()

        if run_analysis:
            if not ticker_input: st.warning("הכנס טיקר"); return

            # Use prefilled data if ticker matches, skip re-fetching
            if prefill and ticker_input==prefill["ticker"]:
                r=prefill
                regime=st.session_state.get("ai_regime") or get_market_regime()
                sectors=st.session_state.get("ai_sectors") or get_sector_ranking()
                fg=get_fear_greed(); pc=get_put_call()
                show_regime(regime)
                show_sentiment_panel(fg=fg,pc=pc)
                if sectors: show_sectors(sectors)
                st.markdown("---")
                st.info(f"♻️ שימוש בנתונים מהסורק — לא נטענו מחדש")
            else:
            ...
            with st.spinner("טוען חדשות ואנליסטים..."):
                ...
                cv_data=get_conviction_data(ticker_input)
                ups=get_recent_upgrades(ticker_input)
            show_conviction_badge(cv_data)
            show_upgrade_badge(ups)
            show_news_card(ticker_input,news_items,cat_sent,cats)
            if api_key:
