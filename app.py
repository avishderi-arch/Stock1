...
def main():
    ...
    # ── High Conviction ───────────────────────────────────────
        ...
        st.info("ניקוד 78+ | ADX 32+ | Volume 2x+ | מומנטום 15%+")
        if st.button("🔥 מצא High Conviction"):
            spy_c=get_spy(); prog=st.progress(0); hc=[]
            for i,t in enumerate(tickers):
                r,_=analyse(t,spy_c,strict=True)
                if r and r["score"]>=78 and r["adx"]>=32 and r["vs"]>=2.0 and r["m3"]>=15:
            ...
            else: st.warning("לא נמצאו High Conviction setups היום.")
    ...
    # ── Multiple ──────────────────────────────────────────────
        ...
        if not tickers: st.warning("הכנס טיקרים"); return
