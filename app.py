...
def analyse_stock(ticker, spy_close, strict=True, sector_score=0):
    try:
        ...
        # VWAP
        tp=(h+l+c)/3
        vwap_val=safe_float((tp*v).rolling(20).sum()/v.rolling(20).sum().replace(0,np.nan).iloc[-1])
        if vwap_val<=0: vwap_val=price
    ...
    if mode=="🔍 סריקה Top-Down (מומלץ)":
        ...
        if st.button("🔍 סרוק Top-Down"):
            ...
            for i,t in enumerate(all_t):
                t_sec=next((s["sector"] for s in top_secs if t in s["stocks"]),"")
